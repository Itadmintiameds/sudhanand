...
const BrandLogosSection = () => {
  const brands = [
    ...
    { 
      ...
      width: 64,
      height: 80,
      containerClass: "w-20 h-28 relative"
    },
    { 
      src: "/home/smt.png", 
      alt: "SMT Logo", 
  ...
  const BrandLogoItem = ({ brand }: { brand: typeof brands[0] }) => {
    const commonClasses = "opacity-60 hover:opacity-100 transition-opacity cursor-pointer hover:scale-105 transform-gpu object-contain";
...
const TimelineSection = () => {
  ...
  const MobileTimelineCard = ({ item, index }: { item: typeof timelineItems[0]; index: number }) => (
    <div key={index} className="w-full max-w-md flex flex-col justify-start items-center gap-1 group transition-all duration-300">
      <div className={`
  ...
  const DesktopTimelineCard = ({ item, index, showDownArrow }: { 
    item: typeof timelineItems[0]; 
    index: number;
    showDownArrow?: boolean;
  }) => (
    <div key={index} className="w-56 h-60 flex flex-col justify-start items-center gap-1 relative group transition-all duration-300 hover:scale-105">
      <div className={`
        w-full h-16 px-2.5 py-4 rounded-tl-2xl rounded-tr-2xl 
        outline outline-1 outline-white/40 backdrop-blur-md 
        flex justify-center items-center
  ...
  const row2 = timelineItems.slice(4, 8);
        ...
        <div className="hidden md:flex flex-col items-center gap-8">
            ...
            {row1.map((item, i, arr) => {
              ...
              return (
                <React.Fragment key={i}>
                  <DesktopTimelineCard item={item} index={i} showDownArrow={isLast} />
                  {i < arr.length - 1 && (
                    <Image 
                      src="/rightarrow.svg" 
          ...
          <div className="flex items-center justify-center gap-4">
            {row2.map((item, i, arr) => {
              ...
              return (
                <React.Fragment key={i}>
                  <DesktopTimelineCard item={item} index={i + 4} showDownArrow={isFirst} />
                  {i < arr.length - 1 && (
                    <Image 
                      src="/leftarrow.svg" 
                      className="w-6 h-6 opacity-80 hover:opacity-100 transition-opacity" 
          ...
          <div className="flex items-center justify-center gap-4">
            {row3.map((item, i, arr) => (
              <React.Fragment key={i}>
                <DesktopTimelineCard item={item} index={i + 8} />
                {i < arr.length - 1 && (
                  <Image 
                    src="/rightarrow.svg" 
                    className="w-6 h-6 opacity-80 hover:opacity-100 transition-opacity" 
